...
IMG ?= ghcr.io/achetronic/hass-mcp:placeholder
...
build: fmt vet ## Build CLI binary.
	go build -o bin/hass-mcp-$(GO_OS)-$(GO_ARCH) cmd/main.go
...
package: ## Package binary.
	...
	@if [ "$(OS)" = "linux" ]; then \
		tar --transform="s/hass-mcp-$(GO_OS)-$(GO_ARCH)/hass-mcp/" -cvzf dist/$(PACKAGE_NAME) -C bin hass-mcp-$(GO_OS)-$(GO_ARCH) -C ../ LICENSE README.md; \
	elif [ "$(OS)" = "darwin" ]; then \
		tar -cvzf dist/$(PACKAGE_NAME) -s '/hass-mcp-$(GO_OS)-$(GO_ARCH)/hass-mcp/' -C bin hass-mcp-$(GO_OS)-$(GO_ARCH) -C ../ LICENSE README.md; \
	else \
